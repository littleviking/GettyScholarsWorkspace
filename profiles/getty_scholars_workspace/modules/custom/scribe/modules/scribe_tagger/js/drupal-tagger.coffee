...
# This file is loaded semi-dynamically, but under
# certain situations Drupal caching can cause it to fire up anyhow
# and it will load on pages even if annotator isn't loaded
# So we check to make sure annotator is there otherwise we completely
# short circuit all of this code
if not Annotator?
  return

# Public: Tags plugin allows users to tag thier annotations with metadata
# stored in an Array on the annotation as tags.
class Annotator.Plugin.DrupalTags extends Annotator.Plugin
  ...
  setAnnotationTags: (field, annotation) =>
    annotation.field_annotation_tags = this.tagListToReferences(@input.val())
  ...
  # Turn a list of tags in array of references to tags
  # This will allow all of the tags to be submitted
  tagListToReferences: (list) ->
    submitTerms = []
    terms = @options.parseTags(list)

    # Create a list of terms to be submitted
    for term in terms
      if Settings.term_map[term]?
        submitTerms.push({
